...
if [[ $HOST = "x86_64-apple-darwin11" ]]; then
    find $TRAVIS_BUILD_DIR -type f | grep -i kingchain-qt.dmg$ | xargs -i cp {} $OUTDIR/bin
    find $TRAVIS_BUILD_DIR -type f | grep -i kingchain-core.dmg$ | xargs -i cp {} $OUTDIR/bin
else
    cp $TRAVIS_BUILD_DIR/src/qt/kingchain-qt $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/qt/kingchain-qt.exe $OUTDIR/bin/ || echo "no QT Wallet"
    cp $TRAVIS_BUILD_DIR/src/kingchaind $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/kingchain.exe $OUTDIR/bin/
    cp $TRAVIS_BUILD_DIR/src/kingchain-cli $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/kingchain-cli.exe $OUTDIR/bin/
