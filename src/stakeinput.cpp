...
// The zKONG block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zKONG is a hash of the serial
...
{
    //Create an output returning the zKONG that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZKONGOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zKONG output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zKONG", __func__);
    ...
    while (it != mintMap.end()) {
        ...
        while (numberToMint > 0) {
            ...
            if (!pwallet->CreateZKONGOutPut(denom, out, dMintReward))
                return error("%s: failed to create zKONG output", __func__);
...
{
    CzKONGTracker* zkongTracker = pwallet->zkongTracker.get();
    CMintMeta meta;
    if (!zkongTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zkongTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a KONGCOIN stake is the outpoint
