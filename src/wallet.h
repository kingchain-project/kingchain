...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 KONGCOIN at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zKONG send
enum ZerocoinSpendStatus {
    ZKONG_SPEND_OKAY = 0,                            // No error
    ZKONG_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZKONG_WALLET_LOCKED = 2,                         // Wallet was locked
    ZKONG_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZKONG_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZKONG_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZKONG_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZKONG_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZKONG_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZKONG_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZKONG_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZKONG_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZKONG_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZKONG_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZKONG_SPENT_USED_ZKONG = 14,                      // Coin has already been spend
    ZKONG_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZKONG_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZKONGOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzkongAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzKONGTracker> zkongTracker;
    ...
    void setZWallet(CzKONGWallet* zwallet)
    {
        zwalletMain = zwallet;
        zkongTracker = std::unique_ptr<CzKONGTracker>(new CzKONGTracker(strWalletFile));
    }

    CzKONGWallet* getZWallet() { return zwalletMain; }
    ...
    /** zKONG reset */
    boost::signals2::signal<void()> NotifyzKONGReset;
