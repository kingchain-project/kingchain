...
#include "config/kingchain-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("kingchain-shutoff");
...
void Shutdown()
{
    ...
    if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
        lightWorker.StopLightZkongThread();
    }
#ifdef ENABLE_WALLET
    delete pwalletMain;
    pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "kingchain.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "kingchaind.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the KONGCOIN and zKONG money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 7777, 10500));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in KONGCOIN/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in KONGCOIN/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, kingchain, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in KONGCOIN/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all KONGCOIN specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-kongstake=<n>", strprintf(_("Enable or disable staking functionality for KONGCOIN inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zkongstake=<n>", strprintf(_("Enable or disable staking functionality for zKONG inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzkong=<n>", strprintf(_("Enable automatic wallet backups triggered after each zKONG minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zkongbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zKONG backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizekingchainamount=<n>     " + strprintf(_("Keep N KONGCOIN anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 7778, 11000));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2019-%i The KONG Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2020-%i The KingChain Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("kingchain-loadblk");
...
/** Sanity checks
 *  Ensure that KONGCOIN is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. KingChain Core is shutting down."));
...
#endif
    // Make sure only a single KONGCOIN process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. KingChain Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("KONGCOIN version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    if (proxyArg != "" && proxyArg != "0") {
        CService proxyAddr;
        if (!Lookup(proxyArg.c_str(), proxyAddr, 7777, fNameLookup)) {
    ...
    if (onionArg != "") {
        if (onionArg == "0") { // Handle -noonion/-onion=0
            ...
            CService onionProxy;
            if (!Lookup(onionArg.c_str(), onionProxy, 7777, fNameLookup)) {
    ...
    //KONGCOIN: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //KONGCOIN specific: zerocoin and spork DB's
                ...
                // KONGCOIN: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    //if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZKONGMinted();
                        RecalculateZKONGSpent();
                    //}
                    RecalculateKONGCOINSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    //if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        ...
                        // KONGCOIN: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of KingChain Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart KingChain Core to complete") << "\n";
        ...
        zwalletMain = new CzKONGWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zKONGWallet
        uiInterface.InitMessage(_("Syncing zKONG wallet..."));

        pwalletMain->InitAutoConvertAddresses();

        bool fEnableZ0cashBackups = GetBoolArg("-backupzkong", true);
        pwalletMain->setZ0cashAutoBackups(fEnableZ0cashBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zkongTracker->Init();
...
//    }
//
//    nAnonymizeKingChainAmount = GetArg("-anonymizekingchainamount", 0);
//    if (nAnonymizeKingChainAmount > 999999) nAnonymizeKingChainAmount = 999999;
//    if (nAnonymizeKingChainAmount < 2) nAnonymizeKingChainAmount = 2;
    ...
    LogPrintf("Anonymize KONGCOIN Amount %d\n", nAnonymizeKingChainAmount);
    ...
    /* Denominations
       ...
       For example:
       1KONGCOIN+1000 == (.1KONGCOIN+100)*10
       10KONGCOIN+10000 == (1KONGCOIN+1000)*10
    */
    ...
    */
    ...
    if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
        ...
        lightWorker.StartLightZkongThread(threadGroup);
    }
