...
#ifndef KONGCOIN_ZKONGWALLET_H
#define KONGCOIN_ZKONGWALLET_H
...
public:
    CzKONGWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZKONG(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZKONG(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //KONGCOIN_ZKONGWALLET_H
