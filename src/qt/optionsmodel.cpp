...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::KONG);
    ...
    if (!settings.contains("nAnonymizeKingChainAmount"))
        settings.setValue("nAnonymizeKingChainAmount", 1000);

    nAnonymizeKingChainAmount = settings.value("nAnonymizeKingChainAmount").toLongLong();
...
// Wallet
    ...
    if (!settings.contains("addrProxy"))
        settings.setValue("addrProxy", "127.0.0.1:7777");
    ...
    if (settings.contains("nAnonymizeKingChainAmount"))
        SoftSetArg("-anonymizekingchainamount", settings.value("nAnonymizeKingChainAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in kingchain.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeKingChainAmount:
            return QVariant(nAnonymizeKingChainAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeKingChainAmount:
            nAnonymizeKingChainAmount = value.toInt();
            settings.setValue("nAnonymizeKingChainAmount", nAnonymizeKingChainAmount);
            emit anonymizeKingChainAmountChanged(nAnonymizeKingChainAmount);
            break;
        case CoinControlFeatures:
