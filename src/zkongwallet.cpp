...
CzKONGWallet::CzKONGWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zkong seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZKONGSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZKONGSeed_deprecated()) {
                    LogPrintf("%s: Updated zKONG seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zkong seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zkong wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzKONGWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZKONGCount(nCountLastUsed);
    else if (!walletdb.ReadZKONGCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzKONGWallet::Lock()
{
    seedMaster = 0;
}

void CzKONGWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzKONGWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZKONG(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzKONGWallet::LoadMintPoolFromDB()
{
...
void CzKONGWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzKONGWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzKONGWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zkongTracker->HasPubcoinHash(pMint.first)) {
...
bool CzKONGWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZKONG(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zkongTracker which also adds to database
    pwalletMain->zkongTracker->Add(dMint, true);
...
void CzKONGWallet::SeedToZKONG(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzKONGWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzKONGWallet::GenerateDeterministicZKONG(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzKONGWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzKONGWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
